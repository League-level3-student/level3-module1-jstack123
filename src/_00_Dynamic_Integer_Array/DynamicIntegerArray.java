...
public class DynamicIntegerArray {
	...
	public void insert(int v, int location) {
		//A. create and initialize a new int array to be one 
		//   element longer than the member array
		int[] secondIntArray = new int[intArray.length+1];
		//B. Make a for loop that iterates through the new array
			...
			for (int i = 0; i < secondIntArray.length; i++) {
				if (i < location) {
					...
					secondIntArray[i] = intArray[i-1];
				} else {
					secondIntArray[i] = v;
				}
				
			}
			intArray = secondIntArray;
	...
	public void remove(int location) {
		...
		int[] lastArray = new int[intArray.length-1]; 
		//B. make a for loop to iterate through the member array
		for (int i = 0; i < intArray.length; i++) {
			if (i < location) {
				lastArray[i] = intArray[i];
			} else if (i > location) {
				lastArray[i-1] = intArray[i];
			} else {
				continue;
			}
		}
